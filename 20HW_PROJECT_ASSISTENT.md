...
- ✅ Реализована автоматическая индексация .md файлов при включении Project helper через MCP tool `index_project_files`
...
1. **Индексация при включении Project Helper:**
   ...
   - MCP сервер:
     - Находит все .md файлы в проекте (рекурсивный обход, игнорирует build, .git, node_modules)
...
2. **Обработка запроса /help:**
   ...
   - MCP сервер:
     ...
     - Обрезает результат до 5 предложений для краткости
     - Возвращает самый релевантный чанк с информацией об источнике
...
4. **Интеграция с RAG:**
   - При включенном Project Helper и Ollama Vector Search:
     ...
     - Все .md файлы проекта доступны для поиска
...
### Проблема: Не находятся .md файлы
**Решение:**
1. Убедитесь, что указан правильный путь к проекту при запуске сервера
2. Проверьте, что файлы не в игнорируемых директориях (build, .git, node_modules, .gradle, .idea)
...
# Проверка скрипта запуска всех серверов
...
```
**Результат:** ✅ Сервер запускается, Ollama доступен
...
MCP Server (ProjectHelperMcpServer)
  ↓
1. findMarkdownFiles() - поиск .md файлов
  ↓
...
- **Индексация выполняется один раз при включении Project Helper** - все .md файлы индексируются и сохраняются в кэше MCP сервера
- При запросе `/help` используются уже проиндексированные данные из кэша (быстрее и эффективнее)
- MCP сервер обрабатывает все .md файлы проекта автоматически
...
- [ ] Добавить кэширование индексированных файлов для ускорения работы
- [ ] Оптимизировать производительность индексации больших проектов
- [ ] Добавить поддержку других форматов файлов (кроме .md)
