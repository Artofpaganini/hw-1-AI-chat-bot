...
fun main(args: Array<String>) {
    ...
    suspend fun generateSummaryFromChunks(
        ...
        val prompt = buildString {
            appendLine("На основании следующих релевантных фрагментов кода/документации ответь на вопрос пользователя полностью и подробно.")
            appendLine("Вопрос: \"$query\"")
            appendLine()
            appendLine("ВАЖНО: Твой ответ должен быть основан ТОЛЬКО на информации из предоставленных фрагментов проекта. Не используй внешние знания.")
            appendLine("Ответ должен отражать всю суть вопроса и включать все релевантные детали из файлов проекта.")
            appendLine()
            ...
            appendLine("- Ответ должен быть ПОЛНЫМ и отражать всю суть вопроса")
            appendLine("- Изложи ответ структурированно (используй нумерацию или маркеры для лучшей читаемости)")
            appendLine("- Используй ТОЛЬКО информацию из предоставленных фрагментов проекта")
            appendLine("- Включи все релевантные детали, которые помогают ответить на вопрос")
            appendLine("- Не добавляй информацию, которой нет в предоставленных фрагментах")
