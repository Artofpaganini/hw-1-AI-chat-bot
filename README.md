...
4. **Использование RAG (Retrieval-Augmented Generation) с Reranking:**
   - После индексации задавайте вопросы в чате
   - **Сценарий 1: Ollama включен (switcher ON)**
     - Ollama используется только для векторного поиска (embeddings через `nomic-embed-text`)
     - Находятся релевантные chunks из проиндексированных документов
     - **С reranking включен (switcher ON):**
       - Chunks фильтруются по коэффициенту похожести (по умолчанию 50%)
       - Используются только chunks с similarity >= threshold
       - Контекст из отфильтрованных chunks передается в AI chat
     - **Без reranking (switcher OFF):**
       - Используются все найденные chunks без фильтрации
       - В конце ответа добавляется фраза "БЕЗ ФИЛЬТРАЦИИ"
...
2. **RAG запрос (Ollama включен):**
   ...
   - **С reranking включен:**
     - Chunks фильтруются по коэффициенту похожести (можно указать любое значение 0-100, по умолчанию 50%)
     - Используются только chunks с similarity >= threshold
     - В конце ответа добавляется "С Ollama и фильтрацией"
   - **Без reranking:**
     - Используются все найденные chunks без фильтрации
     - В конце ответа добавляется "С Ollama и без фильтрацией"
   - Выбираются топ-3 chunks после фильтрации
...
7. **Проверьте работу RAG с Reranking:**
   - **С Ollama и Reranking включенными:**
     ...
     - Укажите коэффициент похожести (можно ввести любое значение 0-100, по умолчанию 50)
     - Задайте вопрос, связанный с содержимым прикрепленного файла
     - AI должен использовать отфильтрованные chunks
