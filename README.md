...
4. **Использование RAG (Retrieval-Augmented Generation):**
   ...
   - **Сценарий 1: Ollama включен (switcher ON)**
     ...
     - В ответе отображается:
       ...
       - Краткое summary каждого chunk'а
   - **Сценарий 2: Ollama выключен (switcher OFF)**
     - AI chat формирует ответ самостоятельно (без Ollama)
...
2. **RAG запрос (Ollama включен):**
   ...
   - Chunks добавляются в контекст и отправляются в Ollama (модель `llama3.2`)
   - Ollama генерирует ответ на основе контекста
   - Для каждого chunk'а генерируется краткое summary
   - Ответ формируется: основной ответ + информация о chunks (номера + summary)
...
7. **Проверьте работу RAG:**
   - **С Ollama включенным:**
     - Задайте вопрос, связанный с содержимым прикрепленного файла
     - AI должен использовать Ollama для генерации ответа
